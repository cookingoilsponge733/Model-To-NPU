...
### Текущая линия APK (v0.4.4) — более плавный preset-only проход по Android UI

Текущая публичная линия `v0.4.4` — это stability-focused follow-up именно для Android-приложения. Ручное редактирование `WxH` теперь скрыто с главного экрана, чтобы генерация оставалась только на списке валидированных preset'ов, preview/final PNG перед показом декодируются в screen-sized bitmap'ы вместо безусловной полной загрузки в UI, а сам APK экспортирует более щадящий QNN-профиль `sustained_high_performance` вместо более агрессивного `burst`, чтобы уменьшить риск общего лага телефона и краша приложения во время генерации. Все shared FIFO / prewarm fixes из `v0.4.3` при этом сохранены; в этой сессии была валидирована сборка APK для новой линии, но свежий phone-side timing run ещё не снимался, поэтому последним документированным end-to-end ориентиром пока остаётся прежний rebuilt-phone результат `75.6 s`.
