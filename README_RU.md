...
### Текущая линия APK (v0.4.5) — откат late-step freeze регрессии

Текущая публичная линия `v0.4.5` сохраняет недавние UI/display улучшения, но откатывает APK-side runtime-изменения, которые с наибольшей вероятностью и приводили к жёстким зависаниям на поздних шагах: foreground-run теперь явно форсирует `SDXL_QNN_SHARED_SERVER=0`, приложение гасит app-open prewarm helper перед реальной генерацией, чтобы в живых не оставалось сразу несколько тяжёлых QNN-владельцев, а экспортируемый из APK perf profile возвращён с `sustained_high_performance` обратно на `burst`. В этой же сессии был записан прямой phone-side validation run с тем же env, который теперь экспортирует APK (`prompt=cat`, `seed=777`, `8` шагов, `CFG=3.5`, `--prog-cfg`): прогон завершился за **29.9 s total** (`UNet 14.891 s`, `VAE 1.942 s`), а проблемная поздняя unguided-часть снова вернулась в ожидаемый класс около **`1.2 s` на шаг** (`step 7 = 1218 ms`) без зависания телефона.
