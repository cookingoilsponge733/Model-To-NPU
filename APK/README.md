...
Текущая документированная версия APK: **`0.2.4-beta`**.
...
Для `v0.2.3` сейчас полезно держать в голове два маркера:

- APK screenshot marker (Live Preview ON): около **78.0 s total**;
- точный runtime-замер с активным HTP backend-extension fast path (Live Preview OFF, тот же `v0.2.3` путь): **62.0 s total** при `seed=777`, `steps=8`, `CFG=3.5`, `--prog-cfg`, со стадиями `CLIP 1.787 s`, `UNet 55.980 s`, `VAE 3.138 s`.
...
Практический смысл такой: если backend-extension fast path не включился или реально не сработал на задеплоенном runtime, полный прогон в классе **~77–80 s** здесь остаётся нормой; маркер **62.0 s** нужно читать как best-known runtime figure именно для случая, где HTP backend extensions действительно активны.
...
- APK `v0.2.4-beta` перед запуском runtime явно экспортирует `SDXL_QNN_USE_MMAP=1`, `SDXL_QNN_PERF_PROFILE=sustained_high_performance`, жёстко отключает daemon fast-path (`SDXL_QNN_USE_DAEMON=0`), включает async/prestage/prewarm-флаги для текущего Python runtime, включает live-температуры, автоматически добавляет `SDXL_QNN_CONFIG_FILE`, если рядом лежат `htp_backend_extensions_lightning.json` и `lib/libQnnHtpNetRunExtensions.so`, уводит временные `WORK_DIR` / preview / output-файлы в app cache и корректно парсит preview-тайминги формата `QNN GPU ...ms`
- APK `v0.2.4-beta` также умеет сначала искать рабочий Python в bundled offline runtime, а если обычный app shell не видит Termux-private `python3`, автоматически переключаться на root shell вместо немого `127`
- если в `phone_gen/lib/` лежит `libsdxl_runtime_accel.so`, APK `v0.2.4-beta` дополнительно прокидывает путь к optional native C runtime accelerator, чтобы scheduler/layout hot spots могли уйти с NumPy на собственный shared library без изменения пользовательского сценария
- `scripts/deploy_to_phone.py` для `v0.2.4-beta` теперь умеет подхватывать `libQnnHtpV79Skel.so` не только из `aarch64-android`, но и из sibling-папки `hexagon-v79/unsigned`, а также деплоить `phone_runtime_accel.py` и optional `libsdxl_runtime_accel.so`, если они заранее собраны локально
