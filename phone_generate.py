...
def _get_qnn_env() -> dict:
    if not _QNN_ENV:
        ...
        _QNN_ENV["LD_LIBRARY_PATH"] = (
            f"{QNN_LIB}:{QNN_BIN_DIR}:{QNN_MODEL_DIR}"
...
def _phone_temp() -> tuple[float | None, str | None]:
    ...
    for zone in range(20):
        try:
            ...
            if any(k in ztype for k in keywords):
                with open(f"/sys/class/thermal/thermal_zone{zone}/temp", "r", encoding="utf-8") as f:
...
def generate(prompt, seed=42, steps=8, cfg_scale=3.5, neg_prompt=None,
    ...
    for si in range(steps):
        ...
        if SHOW_TEMP:
            ...
            if temp is not None and zone is not None:
...
def _preview_step(latents: np.ndarray, step_idx: int, total_steps: int) -> None:
    ...
    out = result[0][0].transpose(1, 2, 0)
    img = np.clip(out / 2.0 + 0.5, 0.0, 1.0)
    img_u8 = (img * 255).astype(np.uint8)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--preview", action="store_true",
                    help="Decode each step with TAESD ONNX on CPU for live preview")
